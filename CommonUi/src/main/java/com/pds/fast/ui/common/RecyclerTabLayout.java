...
public class RecyclerTabLayout extends RecyclerView {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (mIndicatorWidth > 0) {
            int temp = ((right - left) - mIndicatorWidth) / 2;
            left += temp;
            right -= temp;
        }

        int top = getHeight() - mIndicatorHeight - mIndicatorMarginBottom;
        int bottom = getHeight() - mIndicatorMarginBottom;
        canvas.drawRoundRect(left, top, right, bottom, mRoundRadius, mRoundRadius, mIndicatorPaint);
    }
...
//            tabTextView.setTextAppearance(parent.getContext(), mTabTextAppearance);
        ...
        public void onBindViewHolder(ViewHolder holder, int position) {
            ...
            boolean selected = getCurrentIndicatorPosition() == position;
            holder.title.setSelected(selected);
            if (selected) {
                holder.title.setScaleX(1.02f);
                holder.title.setScaleY(1.02f);
                holder.title.setTypeface(Typeface.DEFAULT_BOLD);
            } else {
                holder.title.setScaleX(1f);
                holder.title.setScaleY(1f);
