package com.pds.fast.ui.common.snap;
...
public class PagerSnapWithSpanCountHelper
    ...
    public PagerSnapWithSpanCountHelper(int mSpanCount, boolean mIsEnableCenter) {
    ...
    public int[] calculateDistanceToFinalSnap(
        ...
        if (layoutManager.canScrollHorizontally()) {
            out[0] = mIsEnableCenter ? distanceToCenter(layoutManager, targetView, getHorizontalHelper(layoutManager))
    ...
    * */
    ...
    public View findSnapView(final RecyclerView.LayoutManager layoutManager) {
        if (layoutManager.canScrollVertically()) {
            return mIsEnableCenter ? findCenterView(layoutManager, getVerticalHelper(layoutManager))
                    : findStartView(layoutManager, getVerticalHelper(layoutManager));
        } else if (layoutManager.canScrollHorizontally()) {
            return mIsEnableCenter ? findCenterView(layoutManager, getHorizontalHelper(layoutManager))
    ...
    public int findTargetSnapPosition(final RecyclerView.LayoutManager layoutManager,
        ...
        if (layoutManager.canScrollVertically()) {
            mStartMostChildView = mIsEnableCenter ? findCenterView(layoutManager, getVerticalHelper(layoutManager))
                    : findStartView(layoutManager, getVerticalHelper(layoutManager));
        } else if (layoutManager.canScrollHorizontally()) {
            mStartMostChildView = mIsEnableCenter ? findCenterView(layoutManager, getHorizontalHelper(layoutManager))
                    : findStartView(layoutManager, getHorizontalHelper(layoutManager));
        }
        ...
        return reverseLayout ? (forwardDirection ? centerPosition - mSpanCount : centerPosition)
                : (forwardDirection ? centerPosition + mSpanCount : centerPosition);
    }
