...
public class FastDividerItemDecoration extends RecyclerView.ItemDecoration {
     ...
     */
    public FastDividerItemDecoration(Context context, int orientation, int drawableId) {
        this.setOrientation(orientation);
    ...
    public FastDividerItemDecoration setDividerBoth(int dividerWidth, int dividerHeight) {
        this.setOrientation(BOTH_SET);
    ...
    public void getItemOffsets(Rect outRect, View view, RecyclerView parent, RecyclerView.State state) {
        ...
        switch (mOrientation) {
            ...
            case VERTICAL_LIST:
                childCount -= 1;
                boolean isAddVerticalDivider = isReverseLayout(parent) ? 0 != itemPosition : itemPosition != childCount;
                //水平布局右侧留Margin,如果是最后一列,就不要留Margin了
                outRect.set(0, 0, isAddVerticalDivider ? mDividerWidth : 0, 0);
                break;
            case HORIZONTAL_LIST:
                childCount -= 1;
                boolean isAddHorizontalDivider = isReverseLayout(parent) ? 0 != itemPosition : itemPosition != childCount;
                //垂直布局底部留边，最后一行不留
                outRect.set(0, 0, 0, isAddHorizontalDivider ? mDividerHeight : 0);
                break;
        }
    }
